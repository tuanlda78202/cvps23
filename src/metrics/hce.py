...
def relax_HCE(gt, rs, gt_ske, relax=5, epsilon=2.0):
    ...
    ctrs_FP, hier_FP = cv.findContours(
        FP_.astype(np.uint8), cv.RETR_TREE, cv.CHAIN_APPROX_NONE
    )
    ...
    ctrs_FN, hier_FN = cv.findContours(
        FN_.astype(np.uint8), cv.RETR_TREE, cv.CHAIN_APPROX_NONE
    )
    ## find control points and independent regions for human correction
    bdies_FN, indep_cnt_FN = filter_bdy_cond(
...
def compute_hce(pred_root, gt_root, gt_ske_root):
    ...
    hce_metric = {"names": gt_name_list, "hces": hces}
